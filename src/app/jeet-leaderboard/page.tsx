...
export default function JeetLeaderboardPage() {
  ...
  const exportProfileCard = async (p: Profile) => {
    ...
    const list = (p.mostJeeted || []).slice(0, 3).map((c: any) => typeof c === "string" ? c : c.name).join(" · ") || "—";
  ...
  const uploadAndShareLink = async () => {
    ...
    if (!upload.ok) {
      alert("Share not supported on this platform. Please use Download instead.");
      return;
    }
          ...
          <div id="make-card" className="rounded-xl border border-gray-800 bg-gray-900 p-4 md:order-2">
            ...
            <input type="file" accept="image/*" className="bg-gray-800 rounded px-3 py-2 outline-none" onChange={(e)=>{
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <button className="px-3 py-2 rounded bg-green-600 hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 transition" onClick={uploadAndShareLink}>Share</button>
              </div>
              {generated && (
                <div className="mt-3">
              ...
              {activeTab === "roasts" && (
                <div className="p-4">
                  {selected.roasts && selected.roasts.length > 0 ? (
                    ...
                    <div className="text-sm text-gray-400 p-3">No roasts available yet.</div>
                  )}
                </div>
              )}
